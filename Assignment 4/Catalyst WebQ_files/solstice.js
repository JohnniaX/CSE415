...
Solstice.CompiledTemplates.init = function(namespace, template) {
    var response = Solstice.Remote.run('Solstice', 'compile_template', {'namespace':namespace,'template':template}, null, false);
...
Solstice.newFeature = function(namespace, key, container_id) {
    return new Solstice._newFeature(namespace, key, container_id);
}

Solstice._newFeature = function(namespace, key, container_id) {
    this.namespace = namespace;
    this.key = key;

    // force loading the remote...
    new Solstice.Lang(namespace);

    this.drawLink = function() {
        // old-school remote calls don't allow for overriding success.  till this is re-written as a yui3-style widget, do this hack.
        ...
        var key = this.key;
        var label = Solstice.Lang.getString(this.namespace, 'newfeature-'+key+"-label");
        var title = Solstice.Lang.getString(this.namespace, 'newfeature-'+key+"-title");
        var html = '<a id="'+key+'_open_link" href="#" title="'+title+'">'+label+'</a>';
        ...
        Solstice.Event.add(key+'_open_link', 'click', this.openDialog, this, true);
    };

    this.openDialog = function(e) {
        Solstice.Event.stopEvent(e);
        var key = this.key;
        var menu = Solstice.YahooUI.PopIn.get('new_feature_'+key);
        if (!menu) {
            menu = new YAHOO.widget.Menu('new_feature_'+key, {
                shadow: false,
                classname: 'new-feature-dialog'
            });
            var dialog_content = Solstice.Lang.getString(namespace, 'newfeature-'+key+"-text");
            var url = Solstice.Lang.getString(namespace, 'newfeature-'+key+"-url");
            var url_label = Solstice.Lang.getString(namespace, 'newfeature-'+key+"-url_label");
            var url_title = Solstice.Lang.getString(namespace, 'newfeature-'+key+"-url_title");

            var feature_link = '<a id="'+key+'_survey_link" target="new_feature_feedback_'+key+'" title="'+url_title+'" href="'+url+'">'+url_label+'</a>';
            dialog_content = dialog_content.replace("_URL_", feature_link);
            dialog_content += "<span> <a class='container-close' href='javascript:Solstice.YahooUI.PopIn.get(\"new_feature_"+key+"\").hide();'>Close</a></span><span class='knob_green'> </span>";
            menu.setBody(dialog_content);
            menu.render(Solstice.getAppFormID());
            YAHOO.widget.Overlay.windowResizeEvent.subscribe(function() {this.align(); }, menu, true);
            Solstice.YahooUI.PopIn.registry['new_feature_'+key] = menu;
            Solstice.YahooUI.PopIn.manager.register(menu);
            Solstice.Event.add(key+'_survey_link', 'click', this.showSurvey, this, true);
        }
        menu.cfg.setProperty('context', [document.getElementById(key+'_open_link'), 'tl', 'bl']);
        menu.show();
        document.getElementById(key+'_survey_link').focus();
        //        menu.focus();
        Solstice.Remote.run('Solstice', 'new_feature_log', {data : [this.namespace, key, 'open_dialog']});
    };

    this.showSurvey = function(e) {
        Solstice.Remote.run('Solstice', 'new_feature_log', {data : [this.namespace, this.key, 'show_survey']});
    }
